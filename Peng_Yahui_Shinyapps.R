  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      textInput(inputId = "plot_title",
                ...
                placeholder = "e.g. breed + form + lifestg + special diet"),
...
server <- function(input, output) {
  ...
  #output plot2
  dat2 <- reactive({
    ds2 <- gather(dat1()[,c(3:7)], factor_key=TRUE) %>% na.omit()
  ...
  output$box_2 <- renderPlotly({
    d2 <- dat2()
    d2$key<-factor(d2$key, levels=c("Protein", "Fat", "Omega_6s","Calcium","Phosphorus"))
    p2 <- ggplot(d2, aes(x = key, y=value, fill=key)) +
  ...
  #output plot3
  dat3 <- reactive({
    ds3 <- gather(dat1()[,c(8:9)], factor_key=TRUE) %>% na.omit()
    ds3$key <- as.character(ds3$key)
    ds3 <- ds3 %>% filter(key == "CP_ratio")
    ds3$key <- "Calcium/Phosphorus"
    return(ds3)
  })
  output$box_3 <- renderPlotly({
    d3 <- dat3()
    
    p3 <- ggplot(d3, aes(x = key, y=value, fill=key)) +
  ...
  #output plot4
  dat4 <- reactive({
    ds4 <- gather(dat1()[,c(8:9)], factor_key=TRUE) %>% na.omit()
    ds4$key <- as.character(ds4$key)
    ds4 <- ds4 %>% filter(key == "Glucosamine")
    return(ds4)
  })
  output$box_4 <- renderPlotly({
    d4 <- dat4()
    p4 <- ggplot(d4, aes(x = key, y=value, fill=key)) +
...
#      annotate("text", x = 1, y = 0.6*max(as.numeric(d1$Review_Number)), label = paste(as.character(nrow(d1)),"products"))
#       geom_text(aes(label=d1$Price.per.lb),hjust=0, vjust=0)+
