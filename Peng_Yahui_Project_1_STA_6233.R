...
for (i in 1:as.numeric(number)) {
  ...
  for (j in 1:41) { #The biggest [j] in the CSS selector is 41 in page 1
    tryCatch({
      ...
      brand <- nodes[3] %>%
        html_text() %>% try.NA()
      name <- nodes[2] %>% #Clean titles with irregular prefix
        html_text() %>% {gsub('[[:alpha:]]/d','',.)} %>%
...
stats.clean <- stats
stats.clean$Form <- stats.clean$Form %>% gsub('frozen','freeze-dried',.) %>% gsub('air-dried','dehydrated',.) %>% gsub('powder','food topping',.)
...
stats.clean <- stats.clean %>%
  {.[.$Form!='treats'& !(grepl('supplement',.$Form)&!grepl('Top',.$Name)),]}%>% .[,c(1:3,6,20,21,7:18,4,5)]%>%  .[rowSums(is.na(.)) != ncol(.), ]
review <- setDT(review)[Name %chin% stats.clean$Name]%>%
  arrange(desc(Year),desc(Rating),desc(Brand))
stats.final <- stats.clean[,-c(4,5,19,20,21)]
save(stats.final,file="stats_nutro.Rdata")
...
t.brand <- table(as.character(stats.clean$Brand)) %>%
  as.data.frame()%>% arrange(desc(Freq))
